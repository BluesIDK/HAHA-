...
SELECT EMPLOYEE_NUMBER, 
       REPLICATE(' ', 15 - LEN(FIRST_NAME)) + FIRST_NAME AS FIRST_NAME, 
       LAST_NAME + REPLICATE(' ', 15 - LEN(LAST_NAME)) AS LAST_NAME, 
       FLOOR(DATEDIFF(YEAR, BIRTH_DATE, GETDATE())) AS AGE,
       FLOOR(DATEDIFF(YEAR, HIRE_DATE, GETDATE())) AS SENIORITY
FROM EMPLOYEES
...
WHERE O.CITY = 'Berlin'
  AND P.CATEGORY_CODE = (SELECT CATEGORY_CODE FROM CATEGORIES WHERE CATEGORY_NAME LIKE '%dessert%')
...
WHERE NOT EXISTS (
    ...
    WHERE NOT EXISTS (
        ...
        WHERE OD.PRODUCT_REF = P.PRODUCT_REF
